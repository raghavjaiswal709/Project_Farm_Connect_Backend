...
export const createProductController =async(req,res) => {
    ...
    try {
        ...
        const { category ,title,price, quantity, date, discription, address,state,district } =
      req.fields;
...
export const getProductController = async(req,res)=> {
    try {
        const products = await productmodel.find({}).populate('category','userID').select("-image").sort({createdAt:-1})
        res.status(200).send({
          success:true,
...
export const updateProductController =async(req,res) =>{
  try {
    const { category,userID,title,slug,price, quantity, date, discription, address,state,district } =
  req.fields;
const { image } = req.files;
//alidation
switch (true) {
...
const products =  await productmodel.findByIdAndUpdate(req.params.pid,
  {...req.fields, slug:slugify(title)},{new:true});
if (image) {
  products.image.data = fs.readFileSync(image.path);
  products.image.contentType = image.type;
}
